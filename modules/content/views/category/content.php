<div class="span10" id="datacontent">
    ...
    <ul class="nav">
        ...
        ?>
        <li class="pull-right">
            <a class="dropdown-toggle" href="<?php echo baseUrl?>/content/extend/add?id=<?php echo $id?>&belong=content&back=2" >添加内容模板</a>
        </li>
        <?php
        }
        }
                            ...
                            if($val['value'] == 'delete') {
                                ...
                                <?php if ($v['canDelete'] == 0) { ?>
                                    ...
                                    if ($v['deleteType'] == 1) {
                                        ?>
                                        <a class="btn ajax"
                                           href="<?php echo baseUrl ?>/content/extend/delete?id=<?php echo $v['id'] ?>&status=3">递归删除</a>
                                    <?php
                                    } else {
                                        ?>
                                        <a class="btn ajax"
                                           href="<?php echo baseUrl ?>/content/extend/delete?id=<?php echo $v['id'] ?>&status=1">删除本身</a>
                                        <a class="btn ajax"
                                           href="<?php echo baseUrl ?>/content/extend/delete?id=<?php echo $v['id'] ?>&status=2">子类删除</a>
                                        <a class="btn ajax"
                                           href="<?php echo baseUrl ?>/content/extend/delete?id=<?php echo $v['id'] ?>&status=3">递归删除</a>
                                    <?php
                                    }
