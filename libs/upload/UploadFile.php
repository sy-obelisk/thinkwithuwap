...
class UploadFile {//类定义开始
    ...
    private function _check($arr_file) {
        ...
        if(!$this->_checkSize($arr_file['size'])) {
            $this->_setRes(0,'上传文件失败',array('int_error'=>8));
            return false;
        }

        //检查文件Mime类型
        if(!$this->_checkType($arr_file['type'])) {
            $this->_setRes(0,'上传文件失败',array('int_error'=>9));
            return false;
        }
        //检查文件类型
        if(!$this->_checkExt($arr_file['extension'])) {
            $this->_setRes(0,'上传文件失败',array('int_error'=>10));
            return false;
        }

        //检查是否合法上传
        if(!$this->_checkUpload($arr_file['tmp_name'])) {
            $this->_setRes(0,'上传文件失败',array('int_error'=>11));
            return false;
        }
        return true;
    }
